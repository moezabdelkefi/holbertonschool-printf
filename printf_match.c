...
int print_match(const char *format, print pt[], va_list args)
{
	...
	for (i = 0; format[i] != '\0'; i++)
	{
		if (format[i] == '%')
		{ /*begin when we found % */
			/*and begin we found the fisrt character after percent*/
			for (j = 0; pt[j].data != NULL; j++)
			{
				if (format[i + 1] == pt[j].data[0])
				{ /* search about the character*/
					/* with the struct "pt" and incriment the counter j*/
					val = pt[j].y(args);
					if (val == -1)
						return (-1);
					count += val;
			...
			{ /*begin when the character dosent existe in "pt"*/
				if (format[i + 1] != '\0')
				{ /*print the character percent*/
					/*and after character percent and incriment c with 2 character*/
					_putchar(format[i]);
					_putchar(format[i + 1]);
					count = count + 2;
				}
